...
#include <battery_temperature_data.h>
#include "rtc.h"
#endif
// TINNO END


#if defined(MTK_BQ24157_SUPPORT) || defined(MTK_BQ24296_SUPPORT)
#include "i2c.h"
static struct mt_i2c_t charger_ic_i2c;
#endif
...
// TINNO BEGIN
// Added by liaoye on Nov. 22, 2016 for: compensation soc, get ocv from preloader
...
void bq24296_enable_charger(kal_bool enable);
#endif

#if defined(MTK_BQ24157_SUPPORT)
void bq24157_enable_charger(kal_bool enable);
#endif
...
static void pchr_enable_charger(kal_bool enable) 
{
#if defined(MTK_BQ24296_SUPPORT)
	bq24296_enable_charger(enable);
#endif

#if defined(MTK_BQ24157_SUPPORT)
	bq24157_enable_charger(enable);
#endif

	pchr_turn_on_charging(enable);
}

//get the ibat and vbat after stop charging
static void get_vbat_ibat(void)
{
	int count = 0;

	pchr_enable_charger(KAL_FALSE);
	mdelay(100);

	while(1) {
		G_ibat = get_i_sense_volt(5);
    	G_vbat = get_bat_sense_volt(5);

		if ((G_ibat != 0 && G_vbat != 0) || count > 3)
			break;
		
		count++;
	}

	pchr_enable_charger(KAL_TRUE);
}
#endif	//GET_OCV_FROM_PL
// TINNO END
...
#define charger_ic_i2c_ID	I2C1

void bq24157setDefaultV(void)
{
    U8 write_data[8];
	write_data[0]=0x6;
	write_data[1]=0x7A; //4.4
	
    charger_ic_i2c.id = charger_ic_i2c_ID;
	/* Since i2c will left shift 1 bit, we need to set MT6333 I2C address to >>1 */
    charger_ic_i2c.addr = (bq24157_SLAVE_ADDR_WRITE >> 1);
    charger_ic_i2c.mode = ST_MODE;
    charger_ic_i2c.speed = 100;
    
	i2c_write(&charger_ic_i2c, write_data, 2);
	
	write_data[0]=0x2;
	write_data[1]=0xB6;	//4.4
	i2c_write(&charger_ic_i2c, write_data, 2);	

	write_data[0]=0x1;
	write_data[1]=0xF8;	
	i2c_write(&charger_ic_i2c, write_data, 2);
	print("yixuhong bq24157setDefaultV !\n");
}
...
void bq24157_enable_charger(kal_bool enable)
{
	...
	if (enable == KAL_TRUE) {	//enable charging
		print("\nbq24157_enable_charger --- enable charging\n");
		write_data[0]=0x01;
		write_data[1]=0xf8; 
		i2c_write(&charger_ic_i2c, write_data, 2);
	} else {	//disable charging
		print("\nbq24157_enable_charger --- disable charging\n");
		write_data[0]=0x01;
		write_data[1]=0x36; 
		i2c_write(&charger_ic_i2c, write_data, 2);
	}
...
#if defined(MTK_BQ24296_SUPPORT)  
#define bq24296_SLAVE_ADDR_WRITE 0x6B
#define bq24296_I2C_ID	I2C1

void bq24296setDefaultV(void)
{
    U8 write_data[8];
		
    charger_ic_i2c.id = bq24296_I2C_ID;
    charger_ic_i2c.addr = bq24296_SLAVE_ADDR_WRITE;
    charger_ic_i2c.mode = ST_MODE;
    charger_ic_i2c.speed = 100;

	write_data[0]=0x00;
	write_data[1]=0x32; //pre-charging current 500ma
	i2c_write(&charger_ic_i2c, write_data, 2);
	
	write_data[0]=0x03;
	write_data[1]=0x32; //pre-charging current 512ma
	i2c_write(&charger_ic_i2c, write_data, 2);
	
	print("yixuhong bq24296setDefaultV !\n");
}
...
void bq24296_enable_charger(kal_bool enable)
{
	...
	if (enable == KAL_TRUE) {	//enable charging
		...
		print("\nbq24296_enable_charger --- enable charging\n");
		write_data[0]=0x01;
		write_data[1]=0x1b; 
		i2c_write(&charger_ic_i2c, write_data, 2);
	} else {	//disable charging
		print("\nbq24296_enable_charger --- disable charging\n");
		write_data[0]=0x00;
		write_data[1]=0xb2; //disable power path
		i2c_write(&charger_ic_i2c, write_data, 2);
...
// Added by liaoye on Nov. 22, 2016 for: add function--battery temperature check during bootup
#ifdef FEATURE_CHECK_TEMPERATURE_BOOTUP
static int BattThermistorConverTemp(int Res)
{
	int i = 0;
	int RES1 = 0, RES2 = 0;
...
static int BattVoltToTemp(int dwVolt)
{
	int TRes_temp;
	int TRes;
	int sBaTTMP = -100;
...
int get_bat_temp(void)
{
	...
	while (err_count < 3) {
		volt = PMIC_IMM_GetOneChannelValue(3,5,1);
		print("[PL][%s] temperature volt = %d\n", __FUNCTION__, volt);

		if (volt != -1) {
			return BattVoltToTemp(volt);
		}

		err_count++;
		print("[PL][%s] ERROR: get battery temperature voltage err_count = %d\n",
...
void pl_check_bat_temperature(void)
{
	int temp = get_bat_temp();
	print("[PL][%s]: check battery temperature =%d \n", __FUNCTION__, temp);

	while (temp == ERR_CODE || temp <= CLOD_TEMP_THRESLOD || temp >= HOT_TEMP_THRESLOD) {
		print("[PL][%s]: check battery temperature =%d \n", __FUNCTION__, temp);
	//	mtk_wdt_restart();
		
		//poweroff if charger not exist
...
{
#if defined(MTK_NCP1854_SUPPORT)
	ncp1854_enable_charging(KAL_TRUE);
#else
...
// TINNO END
    ...
    //add microtust tee by yinglong.tang begin
    #ifdef TINNO_FINGERPRINT_SUPPORT
	print("[pmic_init] power on MT6328_PMIC_RG_VGP1_EN_ADDR for fingerprint.\n");
	pmic_config_interface(MT6328_PMIC_RG_VGP1_EN_ADDR, 0x0001, MT6328_PMIC_RG_VGP1_EN_MASK, MT6328_PMIC_RG_VGP1_EN_SHIFT);
    #endif
    //add microtust tee by yinglong.tang end
