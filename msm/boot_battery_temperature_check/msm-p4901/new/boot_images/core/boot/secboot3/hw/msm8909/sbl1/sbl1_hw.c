...
$Header: //components/rel/boot.bf/3.1.2.c2/boot_images/core/boot/secboot3/hw/msm8909/sbl1/sbl1_hw.c#3 $

when       who       what, where, why
--------   ---       ----------------------------------------------------------
02/04/16   sk        changes for CDC values update through CDT
10/05/15   lm        Removed boot_pre_i2c_clock_init() API and added in sbl1_main_ctrl fun 
					 to initialize EEPROM correctly
...
{
  uint32 ddr_cdt_params_size = 0, ddr_cdc_delay_params_size=0;
  struct cdt_info *config_data_table_info;
  uint8 *ddr_cdt_params_ptr,*ddr_cdt_params_ptr1;
  ...
  config_data_table_info = 
   ...
   //reset Shared IMEM CDT DDR parameters
  memset((void*)SHARED_IMEM_BOOT_CDT_BASE, 0, config_data_table_info->size);  
  ...
  ddr_cdt_params_ptr1 = ddr_cdt_params_ptr + ddr_cdt_params_size;
  
  if(*ddr_cdt_params_ptr1!=NULL)
	{
	 ddr_cdt_params_ptr1 = 
                    boot_get_config_data_block(config_data_table_info->cdt_ptr,
                                               CONFIG_DATA_BLOCK_INDEX_V1_BOOT_SETTINGS,
                                               &ddr_cdc_delay_params_size);
     
	 ddr_cdt_params_size += ddr_cdc_delay_params_size;
	 
	 /* Make sure cdt param size is valid */
     BL_VERIFY(((ddr_cdt_params_size) <= CONFIG_DATA_TABLE_MAX_SIZE) &&
            (ddr_cdt_params_ptr1 != NULL) , BL_ERR_SBL );
	}
