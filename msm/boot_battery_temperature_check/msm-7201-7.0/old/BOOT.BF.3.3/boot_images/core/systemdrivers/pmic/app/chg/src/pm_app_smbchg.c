...
$Header: //components/rel/boot.bf/3.3/boot_images/core/systemdrivers/pmic/app/chg/src/pm_app_smbchg.c#8 $
$DateTime: 2016/08/03 03:37:40 $
$Author: pwbldsvc $
 
when       who     what, where, why
--------   ---     ----------------------------------------------------------
07/04/16   pxm     Set USB500 mode if charger type is SDP no matter whether weak battery or not.
...
pm_err_flag_type pm_sbl_chg_check_weak_battery_status(uint32 device_index)
{
    ...
    while(vbatt_weak_status)  //While battery is in weak state
    {
        ...
        /*sunhonghui add for LBCFD begin*/
        if(!first_cycle)
        {
            LOGD("force bootup");
            break; //bootup
        }
        /*sunhonghui add for LBCFD end*/
        //Check if USB charger is SDP
        if (chgr_src_detected == PM_SMBCHG_MISC_SRC_DETECT_SDP) 
        {
