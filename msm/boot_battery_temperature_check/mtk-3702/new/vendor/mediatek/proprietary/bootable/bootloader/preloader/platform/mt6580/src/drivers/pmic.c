...
void bq24158_write_byte(u8 reg,u8 value)
{
      U8 write_data[8];
	...
	write_data[0]=reg;
	write_data[1]=value;
	i2c_write(&bq24158_i2c, write_data, 2);	
}
...
void bq24158_turn_off_charging (void){ 
...
void bq24158_turn_on_charging(void){ 
	...
	bq24158_write_byte(0x00,0xc0);  
	bq24158_write_byte(0x01,0x48);  
	bq24158_write_byte(0x05,0x04); 
	bq24158_write_byte(0x04,0x2a);
}
#endif
...
// Added by liaoye on Nov. 22, 2016 for: add function--battery temperature check during bootup
	        case 5:
	            while(pmic_get_register_value(MT6350_PMIC_RG_ADC_RDY_BATON1_ADDR,MT6350_PMIC_RG_ADC_RDY_BATON1_MASK,MT6350_PMIC_RG_ADC_RDY_BATON1_SHIFT) != 1 );
				    ret_data = pmic_get_register_value(MT6350_PMIC_RG_ADC_OUT_BATON1_ADDR,MT6350_PMIC_RG_ADC_OUT_BATON1_MASK,MT6350_PMIC_RG_ADC_OUT_BATON1_SHIFT);					
	            break;
// TINNO END
			case 6:
	            while(pmic_get_register_value(MT6350_PMIC_RG_ADC_RDY_ISENSE_ADDR,MT6350_PMIC_RG_ADC_RDY_ISENSE_MASK,MT6350_PMIC_RG_ADC_RDY_ISENSE_SHIFT) != 1 );
				    ret_data = pmic_get_register_value(MT6350_PMIC_RG_ADC_OUT_ISENSE_ADDR,MT6350_PMIC_RG_ADC_OUT_ISENSE_MASK,MT6350_PMIC_RG_ADC_OUT_ISENSE_SHIFT);
	            break;
...
int get_bat_sense_volt(int times)
{
    return PMIC_IMM_GetOneChannelValue(7,5,1);
}

int get_i_sense_volt(int times)
{
	return PMIC_IMM_GetOneChannelValue(6,5,1);
}
...
int get_bat_temp(void)
{
	...
	while (err_count < 3) {
		volt = PMIC_IMM_GetOneChannelValue(5,5,1);
...
U32 pmic_init (void)
{
    ...
    //hw_check_battery();
    //printf("[pmic6350_init] hw_check_battery\n");
