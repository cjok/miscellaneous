...
public class SerialManager {
    ...
    public boolean is_serial_init() {
        Log.d("litingnew","is_serial_init");
        try {
            Log.d("litingnew","SerialManager: is_serial_init");
            return mService.is_serial_init();
        } catch (RemoteException e) {
            Log.e("litingnew", "RemoteException in getSerialPorts", e);
            return true;
        }
    }
    public int serial_set_control_speed_config() {
        Log.d("litingnew","serial_set_control_speed_config");
        try {
            return mService.serial_set_control_speed_config(32, 0);
        } catch (RemoteException e) {
            Log.e("litingnew", "RemoteException in serial_set_control_speed_config", e);
            return 0;
        }
    }
    
    public int[] serial_get_control_speed_config() {
        Log.d("litingnew","serial_get_control_speed_config");
        try {
            int[] a = mService.serial_get_control_speed_config();
            for (int i = 0; i < a.length ; i++) {
                Log.d("litingnew","i=  "+i+ "  a[i]  "+a[i]);
            }
            Log.d("litingnew","serial_get_control_speed_config   a[0]    "+a[0]+"   a[1]   "+a[1]);
            return a;
        } catch (RemoteException e) {
            Log.e("litingnew", "RemoteException in serial_get_control_speed_config", e);
            return null;
        }
    }
}
